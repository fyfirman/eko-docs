...
- For **Chinese** users: If it's inconvenient to obtain an API key from the OpenAI platform, consider using ChatGPT mirror sites or services (such as [ZetaTechs API](https://api.zetatechs.com/)), and then replace the *Base URL* and *API key* with the corresponding values.
